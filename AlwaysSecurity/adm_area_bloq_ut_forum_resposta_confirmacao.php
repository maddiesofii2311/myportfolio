...
<h2>CONFIRMAÇÃO DE BLOQUEIO DE UTILIZADOR</h2>



<?php
	
	$login_resposta = $_GET['login_resposta'];

	//echo 'login_resposta:'.$login_resposta;	
	$cod_top = $_GET['cod_top'];


if(!$login_resposta || !$cod_top)
{
	echo'Atenção! Utilizador não identificado! Volte atrás e tente de novo!';
	exit;
}


//Confirmar desejo de bloqueio

// Function defnition
function funcao_confirm($message, $cod_top)
{   
    
$caminho="adm_area_bloq_ut_forum_resposta.php?login_resposta=$message&cod_top=$cod_top";
$caminho2="forum_respostas_adm.php?cod_top=$cod_top";
//echo"caminho:".$caminho;
    ...
    echo "  if (confirm('Deseja mesmo bloquear ".$message."?')) {";
    echo '    document.location ="'.$caminho.'";';
    echo '  }';
    echo 'else {';

	echo '    document.location ="'.$caminho2.'";';
	echo '}';
...
// Function call
funcao_confirm($login_resposta, $cod_top);
